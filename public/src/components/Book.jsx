...
export default function Book(props) {
  ...
  let [ownsBook, setOwnsBook] = useState(false);
  ...
  useEffect(() => {
    ...
    if (props.data.userid === user?.user?.id) setOwnsBook(true);
    ...
    <div className="flex gap-4 w-full">
      ...
      {deleteModal && user && (
        <Modal show={deleteModal} onHide={() => setDeleteModal(false)} backdrop="static">
          ...
          <Modal.Body>
            <b>{props.data.title}</b> will be permanently deleted, are you sure?
          </Modal.Body>
          <Modal.Footer>
            <button
      ...
      <div className="content flex flex-col w-1/2">
        ...
        {ownsBook && <button
          className={`btn--secondary mt-auto py-1 w-full rounded-full`}
          onClick={() => {
            setDeleteModal(true)
          }}
          href="#"
